...
| Phase 6 Managed chat and recovery | 32% | Core chat routing outcomes, opt-in steering hook, execution-like request hold-for-confirmation behavior, pending-approval workflow state, typed confirmation consumption, stronger approval classifier, and boundary-injected local permissions exist. | Intent detector split, FlowDesk card UX, retry/resume/abort affordances, and blocking conformance remain missing. |
