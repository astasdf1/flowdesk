...
| Phase 3 OpenCode plugin command path | 68% | Plugin package, FDS-1 schema conversion probe, command-backed handler evaluators, local non-dispatch adapter, opt-in durable `.flowdesk` state materialization wired through server options, fail-closed durable write handling, tool-boundary injected local permissions, portable `/flowdesk-*` command file materialization, typed-bound Release 1 bootstrap installer library with derived profile-root binding, installer-only durable pending/consumed confirmation ledger, one-shot confirmation consumption, symlink-hardened ledger path checks, and command-file rollback on durable artifact or ledger failure, `flowdesk_chat_intake`, opt-in text-only `chat.message` FlowDesk card steering, confirmation-before-run steering guard, pending-approval workflow state writes, and typed-approval chat-run acceptance exist. Tests pass. | Production registration remains blocked, default server is still inert doctor-only, full user-ready install/bootstrap CLI/profile wiring is incomplete, and production adapter promotion remains incomplete. |
...
10. The Release 1 bootstrap installer now persists consumed installer confirmations in a lightweight durable ledger under `.flowdesk/bootstrap/confirmations/<confirmationRef>.json`, rejects consumed or binding-mismatched ledgers before writes, stores only a typed phrase hash, and fails closed with command rollback if ledger persistence fails. Remaining installer work is CLI/profile wiring and user-facing bootstrap flow integration, not cross-process consumed-confirmation replay prevention.
...
11. Critical review on 2026-05-19 blocked the durable confirmation ledger patch for Release 1 readiness: the ledger commit currently happens after complete bootstrap artifacts are written, so a ledger write failure can leave a completed bootstrap report without a consumed-confirmation ledger, and ledger path safety still needs parent/symlink hardening evidence.
