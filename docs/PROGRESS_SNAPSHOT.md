...
9. Critical review on 2026-05-19 found and the follow-up patch resolved the remaining bootstrap installer profile-binding blocker: `profileRootRef` is now derived from the actual selected `profileRootDir`, replay to another profile root is rejected before writes, and successful confirmation refs are consumed once per installer process.
