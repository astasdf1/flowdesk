...
| Phase 6 Managed chat and recovery | 38% | Core chat routing outcomes, opt-in steering hook, visible text-only FlowDesk card suggestions, execution-like request hold-for-confirmation behavior, pending-approval workflow state, typed confirmation consumption, stronger approval classifier, boundary-injected local permissions, and schema-valid chat-routed retry/abort/resume/usage/export-debug diagnostics exist. | Intent detector split, broader abnormal-use recovery UX, and blocking conformance remain missing. |
