...
| Phase 4 OpenCode conformance | 35% | OpenCode 1.14.40 PoC, local OpenCode 1.14.50 / `@opencode-ai/plugin@1.3.12` steering inspection, FDS-1 schema-conversion evidence, and Release 1 packaging/bootstrap smoke evidence exist. | Command alias proof, blocking/no-reply proof, runtime echo, event telemetry, lane observability, and broader pinned-version conformance evidence remain incomplete for later gates. |
...
Release 1 foundations are substantially implemented, with default safe local command-backed chat usable without opt-in, Release 1 minimum tools promoted to production-eligible non-dispatch command-backed registration, package dry-run smoke evidence recorded, and user-facing bootstrap docs updated. Public release packaging approval remains outside the current local workspace scope, while real dispatch/provider/runtime/lane/fallback/hard-chat authority remains later-release scope.
