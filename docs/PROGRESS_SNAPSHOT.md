...
| Phase 6 Managed chat and recovery | 34% | Core chat routing outcomes, opt-in steering hook, visible text-only FlowDesk card suggestions, execution-like request hold-for-confirmation behavior, pending-approval workflow state, typed confirmation consumption, stronger approval classifier, and boundary-injected local permissions exist. | Intent detector split, fuller retry/resume/abort affordances, and blocking conformance remain missing. |
