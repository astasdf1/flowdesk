...
| Phase 2 Policy, usage, audit | 60% | Config/policy/effective-policy, non-dispatch permission, provider health/usage fail-closed helpers, audit/debug write-intent tests exist. | Real config loading, durable audit writer, provider collectors, and debug bundle implementation remain partial. |
| Phase 3 OpenCode plugin command path | 55% | Plugin package, FDS-1 schema conversion probe, command-backed handler evaluators, local non-dispatch adapter, tool-boundary injected local permissions, `flowdesk_chat_intake`, opt-in text-only `chat.message` FlowDesk card steering, confirmation-before-run steering guard, pending-approval workflow state writes, and typed-approval chat-run acceptance exist. Tests pass. | Production registration remains blocked, default server is still inert doctor-only, durable state/commands/user-ready install are incomplete, and production adapter promotion remains incomplete. |
...
| Phase 6 Managed chat and recovery | 36% | Core chat routing outcomes, opt-in steering hook, visible text-only FlowDesk card suggestions, execution-like request hold-for-confirmation behavior, pending-approval workflow state, typed confirmation consumption, stronger approval classifier, boundary-injected local permissions, and schema-valid chat-routed retry/abort diagnostics exist. | Intent detector split, fuller resume/usage/debug affordances, and blocking conformance remain missing. |
