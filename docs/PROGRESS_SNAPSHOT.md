...
| Phase 6 Managed chat and recovery | 46% | Core chat routing outcomes, opt-in steering hook, visible text-only FlowDesk card suggestions with confirmation codes, in-memory duplicate suppression for repeated non-confirmation steering cards, execution-like request hold-for-confirmation behavior, pending-approval workflow state, typed confirmation consumption, TTL/scoped/one-shot/cancellable pending confirmation gating in the local adapter, stronger approval classifier, boundary-injected local permissions, and schema-valid chat-routed retry/abort/resume/usage/export-debug diagnostics exist. | Intent detector split, broader abnormal-use recovery UX, durable suggestion preferences, and blocking conformance remain missing. |
