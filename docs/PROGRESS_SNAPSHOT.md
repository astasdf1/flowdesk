...
| Phase 3 OpenCode plugin command path | 45% | Plugin package, FDS-1 schema conversion probe, command-backed handler evaluators, local non-dispatch adapter, `flowdesk_chat_intake`, and opt-in `chat.message` steering exist. Tests pass. | Production registration remains blocked, default server is still inert doctor-only, durable state/commands/user-ready install are incomplete, and chat-routed run currently needs confirmation redesign. |
...
| Phase 6 Managed chat and recovery | 20% | Core chat routing outcomes and opt-in steering hook exist. | Intent detector split, FlowDesk card UX, confirmation state, approval classifier, retry/resume/abort affordances, and blocking conformance remain missing. |
