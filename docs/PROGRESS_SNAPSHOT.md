...
| Phase 3 OpenCode plugin command path | 50% | Plugin package, FDS-1 schema conversion probe, command-backed handler evaluators, local non-dispatch adapter, `flowdesk_chat_intake`, opt-in `chat.message` steering, confirmation-before-run steering guard, and pending-approval workflow state writes exist. Tests pass. | Production registration remains blocked, default server is still inert doctor-only, durable state/commands/user-ready install are incomplete, and confirmed run acceptance remains incomplete. |
...
| Phase 6 Managed chat and recovery | 25% | Core chat routing outcomes, opt-in steering hook, execution-like request hold-for-confirmation behavior, and pending-approval workflow state exist. | Intent detector split, FlowDesk card UX, typed confirmation consumption, approval classifier, retry/resume/abort affordances, and blocking conformance remain missing. |
