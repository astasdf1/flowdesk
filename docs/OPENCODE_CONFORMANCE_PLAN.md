...
7. Any external OpenUsage-compatible local API or cache is opt-in, loopback-only, version-pinned, redacted, and advisory to FlowDesk validation. It cannot replace provider-native usage evidence unless a pinned conformance artifact proves the source is authoritative for the exact provider/model family and scope.
...
9. Provider-native usage collectors may use documented response rate-limit headers, documented provider admin usage/cost APIs, documented provider rate-limit APIs, or documented cloud quota/monitoring APIs only when the credential scope and model-family mapping are proven. OpenCode SDK response headers that omit provider rate-limit data do not satisfy the usage gate.
...
| Provider usage source research | OpenAI and Anthropic expose documented admin/rate-limit/header surfaces; Google Cloud exposes Cloud Quotas/Monitoring surfaces for supported services; this local shell has no provider/admin keys and OpenCode SDK responses do not forward provider rate-limit headers | collector design path known; local usage gate unsatisfied |
| Release 2 audit/gate smoke | FlowDesk-bound audit write intent can write a redacted audit record while authority flags remain false; missing usage evidence still blocks the managed-dispatch beta gate; follow-up smoke proves durable audit write before live OpenAI SDK prompt | fail-closed gate evidence plus live-call audit ordering smoke |
