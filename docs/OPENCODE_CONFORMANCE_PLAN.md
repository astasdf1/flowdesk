...
OpenCode 1.14.40 local isolated PoC completed on 2026-05-15, with an FDS-1 runtime-closed schema conversion probe added on 2026-05-18 and Release 1 packaging/bootstrap smoke evidence added on 2026-05-19. That Release 1 evidence supports a General-Use MVP with chat-routed command-backed workflows, tested hook-harness containment surfaces, FDS-1 compatibility through FlowDesk runtime-closed validation, package/bin smoke, public registry installation for version `0.1.0`, and non-dispatch command-backed production registration. Release 3 (Operational Intelligence) is now complete, with full verification of score-ledger, ledger-writer, and GH-publisher components.

Separate Release 2 evidence includes core/adapter fake-client smoke evidence and live OpenCode 1.14.50 provider smoke evidence added on 2026-05-19. That later-gate evidence proves an opt-in adapter boundary can fail closed before dispatch and call the injected OpenCode SDK `session.prompt`/`promptAsync` surface only after all core gate evidence is present. The live smoke proves OpenAI provider dispatch through the official SDK surface and exposes provider/model identity in the response; direct CLI diagnostics can reach tested Anthropic Sonnet/Opus ids, pure headless SDK mode fails for Anthropic because it skips the local external Anthropic auth/provider plugin, and non-pure headless SDK mode succeeds for Anthropic Sonnet. OpenCode model catalog metadata is available, but it does not expose provider-native quota/reset authority or forward provider rate-limit headers through the SDK response. A FlowDesk-bound audit/gate smoke remains fail-closed when usage evidence is missing, a live-call ordering smoke proves durable audit write before live OpenAI SDK prompt, runtime echo/telemetry smoke validates redacted SDK response/session correlation against the current schemas, internal collector unit evidence now covers Claude OAuth usage, Codex/OpenAI live usage, and Gemini Code Assist quota, configured-verification result validation is wired into the opt-in production enablement evaluator and doctor diagnostics, sanitized-auth capture result validation gates token-free durable raw-plugin-object sanitization evidence, token-free external-auth/provider policy result validation is wired into the same fail-closed diagnostics path, production approval decision diagnostics are doctor-visible only after core validation, and the managed-dispatch beta code path now includes production approval source, dispatch attempt manifest, idempotency/reservation, production enablement, adapter, and terminal linkage/reload validation artifacts. Default production dispatch, hard main-chat suppression, provider/API/model managed fallback, production approval issuance/source authority, live credentialed usage conformance, and trusted platform-internal runtime conformance remain gated. See `conformance/2026-05-15-opencode-1.14.40-poc.md`, `conformance/2026-05-18-fds1-schema-conversion-probe.md`, `conformance/2026-05-19-release1-packaging-bootstrap-smoke.md`, `conformance/2026-05-19-release2-managed-dispatch-beta-core-adapter-smoke.md`, `conformance/2026-05-19-release2-live-opencode-provider-smoke.md`, `conformance/2026-05-21-release2-configured-verification-gate.md`, `conformance/2026-05-21-release2-sanitized-auth-capture-gate.md`, `conformance/2026-05-21-release2-external-auth-provider-policy-gate.md`, and `conformance/2026-05-21-release2-production-approval-state-gate.md`.
...
Top-tier multi-perspective internal reviewer lane artifacts are present for the Release 2.5 entry gate, including exact model availability cache use, quick reviewer fan-out planning, runtime reviewer execution bridge integration, typed top-tier reviewer verdict contracts, durable reviewer verdict materialization, and same-model multi-perspective handling when only one highest-tier model is available. This does not by itself prove live credentialed conformance for every registered highest-tier provider, default production reviewer orchestration, arbitrary unapproved agent/model override lanes, or release approval outside the explicit opt-in reviewer path.

### Phase 7 Operational Intelligence (OI) Completion

Operational Intelligence implementation completed on 2026-06-11 with all 20 tasks and 12 exit criteria verified. Key conformance items:

1. **OI Ledger Writer Atomic Write Validation**: RFC 8785 JCS canonicalizer integrated; atomic JSONL partition writes with manifest hash chains verified.
2. **Sealed Partition Immutability**: Active raw partitions excluded from reusable rollups; sealed partition rotation and retention proven.
3. **Rollup Safety (EC#8/#9)**: Duplicate event and partition conflict handling verified; normalized aggregate snapshots (weighted means, percentiles, confidence intervals) proven advisory-only.
4. **GitHub Publisher Sealed Partition Gate**: GitHub Actions OIDC authentication, least-privilege workflow permissions, and import manifest validation verified. External managed database cutover and idempotent replay semantics proven.

OI tooling is advisory-only and non-dispatch. Score snapshots cannot make ineligible candidates eligible, override Guard, reduce verification, skip approval, or override usage/Plugin-SDK-compatibility checks. Multi-model proposal fan-out requires explicit user opt-in, fresh provider-native usage, and policy-controlled cadence. All Phase 7 evidence is redacted-safe and contains no forbidden persisted payload classes.
...
Remote connector installation is itself a privileged setup action. FlowDesk may recommend or prepare installation steps for tools such as `gh`, cloud CLIs, database clients, or MCP connector packages, but it must not install, enable, or authenticate them without explicit user approval bound to the active profile, connector kind, requested package/tool/source, expected version or digest where available, and rollback/removal guidance. FlowDesk should not proliferate direct per-service write adapters as the default design; real execution belongs behind the generic profile/recipe gateway.
...
2. `steering`: FlowDesk can mutate or throw from chat hooks to steer normal requests into a FlowDesk envelope, but cannot prove main-chat cancellation or SDK-scoped noReply control.
...
| Review lanes | every registered highest-tier available reviewer/model binding as canonical `reviewer` extensions, with same-model multi-agent perspective assignment when only one top model is registered | No for Release 1 default path; Release 2.5 artifacts present for opt-in reviewer path | Release 2.5 gate artifacts present; live/full-provider conformance still bounded by available evidence | Release 2.5/3 gate artifacts present; default production promotion still approval-gated |
| Review lanes | provider-qualified concrete model ids, fresh auth/usage/quota evidence, plugin-observed lifecycle/result/status refs, output schema, top-tier eligibility, perspective separation, and no silent fallback | No for Release 1 default path | Implemented for opt-in reviewer/gate paths where matching evidence exists; missing evidence blocks | Implemented for opt-in reviewer/gate paths where matching evidence exists; missing evidence blocks |
| Overrides | arbitrary agent/model override constrained by binding registry and Guard evidence | No for Release 1 default path | Constraint artifacts present; unregistered or missing-evidence overrides remain blocked | Constraint artifacts present; unregistered or missing-evidence overrides remain blocked |
...
| Top-tier multi-perspective internal review lanes | Release 2.5 artifacts now cover quick reviewer fan-out, exact model availability cache usage, same-model multi-perspective assignment, runtime reviewer execution bridge integration, typed verdict validation, durable reviewer verdict materialization, and incomplete fan-out blocking. Live credentialed coverage for every registered highest-tier provider and default production promotion remain bounded by available evidence and release approval. | opt-in reviewer path implemented/verified in code; full live conformance and default production promotion remain gated |
