...
| Anthropic diagnostic provider smoke | Direct CLI diagnostics succeed for tested Sonnet/Opus ids; tested Haiku ids return provider 404 model-not-found; headless SDK prompts for tested Sonnet/Opus ids return `UnknownError` | provider reachable, SDK boundary unresolved |
...
| Release 2 audit/gate smoke | FlowDesk-bound audit write intent can write a redacted audit record while authority flags remain false; missing usage evidence still blocks the managed-dispatch beta gate | fail-closed gate evidence; not live-call audit ordering |
