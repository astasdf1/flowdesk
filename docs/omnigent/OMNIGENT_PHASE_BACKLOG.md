...
- **완료(P1 로버스트니스 배치, 2026-07-02)**: guard cache pruning — append 시 만료 record prune + 1시간 이상 stale `.tmp` cleanup(크래시 잔여물); model-id drift canary — `test_registry_model_ids_conform_to_family_prefixes`(family prefix 정합) + `OMNIGENT_SETUP.md`에 수동-검증 산물/갱신 책임 문서화; event-shape 계약 테스트 — `test_guard_reads_canonical_sys_session_send_event_shape`로 guard가 의존하는 `sys_session_send` arg shape(agent/args.task_id·title/args.model·args.harness) 추출을 고정. 검증: Python 102 OK.
- **부분**: guard cache 무결성(same-UID poisoning은 근본적으로 남음 — session-binding/HMAC 미도입, "best-effort" 문서 유지); parity fixture(allowed_models·preferred+tier·dispatchable=false·expires_at 케이스 미추가); event-shape 계약의 Omnigent 버전 핀 매트릭스 미완(shape 추출 canary만 존재); guard 실패 모드 매트릭스의 `OMNIGENT_SAFETY_RULES.md` 반영 미완.
- **완료(P1 마무리 배치, 2026-07-02)**: `ts_cli` bridge 운명 결정 — [ADR 0004](../adr/0004-omnigent-ts-cli-bridge-status.md)(experimental·opt-in·non-default 유지 + 명시적 removal trigger); template manifest+drift — `template_installer`에 `.flowdesk-template-manifest.json`(버전+파일 checksum) 기록 + `check_templates()`/`--check` drift 리포트(modified/missing/version_drift/not_installed); 정식 설치경로 선언 — `flowdesk-omnigent-mcp` 콘솔 스크립트를 유일 supported invocation으로 명문화, `examples/omnigent-flowdesk-mcp` 및 `OMNIGENT_MCP_OPERATION.md`에서 머신-특정 절대경로 제거(python -m+PYTHONPATH은 dev-only fallback로 격하).
- **미착수(P1 잔여)**: guard cache 다중세션 격리(근본 해결 upstream hook), live E2E smoke, Omnigent 버전 핀 매트릭스.
