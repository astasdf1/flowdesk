...
FlowDesk는 Omnigent의 guardrail/policy를 우회하거나 강제하지 않는다.
...
### 8. Phase 1은 advisory-only다

`flowdesk.omnigent_selection.v1` 결과는 Omnigent dispatch gate가 아니다.
Phase 1에서는 trace/evidence로 "선택 후 실행"을 관찰할 수 있지만,
blocked/non-dispatchable 결과를 기계적으로 막는 safety boundary라고 주장하지 않는다.
그 주장은 별도 guardrail 또는 trace-verifier가 구현되고 리뷰된 뒤에만 가능하다.
