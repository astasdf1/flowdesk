...
Release 3 adds advisory evaluation, workflow proposal optimization, multi-model proposal fan-out behind surplus usage gates, GitHub-backed score ledger support, richer operational recovery, and source-grounded reference-pack workflows without weakening Guard or turning specialist output into professional signoff.
...
9. Implement canonical agent profiles with `reviewer` as the only canonical review id, using the Agent Profile Authoring Standard in the implementation spec.
