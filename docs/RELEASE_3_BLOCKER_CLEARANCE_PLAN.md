...
| 4 | Chat hook hard-control uncertainty | R1/R2 / Phase 6 | `chat.message` steering appends visible text and emits no `noReply`, `cancel`, or `stop`. Separate SDK surfaces for `permission.ask` deny, `session.abort`, and prompt `noReply` are now typed and wrapped behind explicit FlowDesk decisions. | E2E proof of no duplicate assistant reply, hard suppression, pending tool abort, cleanup, and audit transitions remains required before any `chat.message` hard-control claim. SDK-level controls must stay distinct from hook-level hard-chat authority. | Hook harness, SDK control adapters, chat conformance docs, threat model. | Chat hook mutation/throw/no-reply/cancel probe plus SDK-control adapter tests. | Required before enabling hard chat no-reply/cancel; not required for explicit SDK abort/permission/noReply helper tests. |
...
2. Keep SDK-level `permission.ask`, `session.abort`, and prompt `noReply` separated from `chat.message` hard-control. Probe `chat.message` mutation/throw/no-reply/cancel behavior under OpenCode 1.15 to either prove or keep blocked the hard chat-control authority (blocker #4). Timeout, malformed output, null output, and unsupported return fields must all map to explicit denied/blocked results, never implicit allow.
...
5. Keep external writes limited to controlled redacted targets with explicit user confirmation. The local writers may materialize only `release_conformance_doc` targets through the controlled docs path or `redacted_audit_export` targets through the controlled `.flowdesk/sessions/<workflow_id>/redacted-audit/<target_ref>.json` path, each with reloadable ledger evidence. GitHub, connector, storage, database, URL, raw-path, and broader external ledger targets remain blocked unless a later dedicated write connector gate is designed and proven.
