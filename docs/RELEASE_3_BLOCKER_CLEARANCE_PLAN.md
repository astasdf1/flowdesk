...
2. Treat SDK-level `permission.ask`, `session.abort`, and prompt `noReply` as the completed SDK-scoped chat/session control slice. Keep them separated from `chat.message` hook hard suppression. Probe `chat.message` mutation/throw/no-reply/cancel behavior only if a future OpenCode hook boundary appears or a product requirement explicitly needs hook-level suppression; timeout, malformed output, null output, and unsupported return fields must continue to map to explicit denied/blocked results, never implicit allow.
