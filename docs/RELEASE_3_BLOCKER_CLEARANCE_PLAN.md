...
4. Keep fallback/reselection as reselection plus full re-gate, not automatic provider switching. The local adapter may return `regate_required`, and the managed fallback orchestrator may return `regate_plan_ready`, but the new attempt must still pass the managed-dispatch gate before any provider call.
