...
Natural-language provider-calling tools are explicit opt-in helper surfaces, not default Release 1 dispatch. In particular, `flowdesk_quick_reviewer_run` may make real reviewer provider calls only when the plugin profile enables `quickReviewerRun` and the tool invocation includes both `developerModeAcknowledged=true` and `allowProviderCall=true`. This helper cannot approve dispatch, switch providers, bypass Guard, or turn reviewer output into authority. Other Release 1 natural-language tools remain diagnostic/planning/status surfaces with provider/runtime/lane-launch authority disabled.

Workflow generation in Release 1 is allowed to decompose an accepted plan request into multiple deterministic, schema-valid, command-backed steps derived from redacted request risk and taxonomy. Generated steps are planning/status artifacts only; they must not launch OpenCode lanes, call providers, infer approval, or widen safe next actions beyond the existing guarded command boundary.

Usage-aware model selection is performance-first. Release 2+ selection may use remaining usage only inside a candidate set that already passed suitability, capability, policy, runtime compatibility, provider-health, and exact-binding checks. Usage pressure may break ties or preserve scarce quota, but it must not select an unsuitable lower-tier model or silently substitute a model/provider.
...
18. Plugin verification boundary. FlowDesk completion criteria require only plugin-verifiable evidence: what it asked OpenCode to do, what plugin code persisted, and what OpenCode returned through exposed plugin/SDK surfaces. OpenCode platform-internal facts such as trusted/attested runtime echo issuer, platform telemetry correlation, lane conformance, usage authority attestation, or internal binary behavior are outside the plugin verification boundary and are not gate blockers. FlowDesk may surface those facts only as non-gating diagnostic scope disclosures, must not fabricate or self-attest them, and records `attestation_scope=plugin_observed_only` on completion records.
...
5. **Conformance**: repeatable evidence that a pinned OpenCode version supports the plugin/SDK-observable hook, command, chat steering, tool-schema, SDK adapter, session/message, lifecycle/result/status, fail-closed, and durable evidence behavior FlowDesk depends on. Conformance enables or disables modes; it does not approve individual dispatches or attest platform-internal execution truth.
...
4. SDK adapter compatibility and plugin-observed lifecycle/result/status conformance are recorded before any real dispatch path is implemented; OpenCode platform-internal runtime echo, telemetry correlation, lane conformance, usage-authority attestation, and execution-mode proof remain outside the plugin boundary and surface only as non-gating diagnostic scope.
5. A single low-risk `real-opencode-dispatch` beta path is promoted only after the plugin-verifiable dispatch bundle passes: configured authorization; concrete provider/model/agent binding plus policy eligibility; fresh plugin-verifiable usage/provider health; sanitized auth capture; external auth/provider policy result; configured verification / SDK compatibility result; consumed Guard/user approval bound to the exact request; durable pre-dispatch audit with redaction validation; dispatch idempotency/reservation; intended injected SDK dispatch path plus adapter capability; observed lifecycle/result/status evidence with terminal semantics; and durable evidence reload plus cross-reference validation. FlowDesk completion records carry `attestation_scope=plugin_observed_only`; platform-internal execution facts are non-gating diagnostics.
...
Real dispatch requires the passing 12-item plugin-verifiable dispatch bundle and redaction-safe observed binding/lifecycle/result/status evidence from the intended OpenCode plugin/SDK path. Platform-internal execution facts such as trusted runtime echo issuer, platform telemetry correlation, lane conformance, account-scope usage authority, or execution mode are not required for completion; they remain non-gating observed diagnostics and must not be fabricated. Without the plugin-verifiable bundle, `/flowdesk-run` may only perform guarded dry-run or fake-runtime dispatch according to release mode.
...
2. The launch request must name an explicit FlowDesk agent profile id, a concrete provider-qualified model id, the runtime model id where OpenCode uses a different value, and the binding registry entry that allowed the pair. Unqualified aliases or abstract labels such as `Claude Opus`, `GPT`, `Gemini Pro`, `opus`, `gpt`, `gemini`, `best`, `latest`, or provider-hidden gateways are not production bindings.
3. Exact model availability discovery is cached for one local day per active profile, registry, Policy Pack, OpenCode version, and account/auth boundary. Same-day review planning reuses the redacted cache; FlowDesk refreshes early only when one of those inputs changes. If no current cache proves exact available model ids, multi-model review must block or ask for discovery rather than substituting abstract model labels.
4. The arbitrary override path is allowed only as a guarded override: the requested agent/model pair must exist in the model binding registry, pass Policy Pack constraints, pass fresh auth/usage/quota and Provider Health Snapshot gates when provider/model selection is involved, and produce runtime echo plus telemetry that matches the approved binding.
5. Override lanes cannot silently fall back, downgrade, substitute a model, change provider, or change agent class. A mismatch, unavailable model, stale usage snapshot, missing auth, missing quota evidence, or missing echo blocks the lane and returns a safe next action.
6. Reviewer lanes return typed critical review outputs only. They may report findings, uncertainty, required fixes, and schema-valid verdict labels, but they do not approve dispatch, replace Guard, replace configured verification, or self-approve the work they review.
...
5. Real dispatch remains disabled until the plugin-verifiable real-dispatch gate has real evidence for the full 12-item completion bundle, including authorization, exact binding/policy eligibility, fresh usage/provider health, configured verification or SDK compatibility, approval, durable pre-dispatch audit, idempotency/reservation, intended SDK dispatch path plus adapter capability, terminal lifecycle/result/status evidence, and durable reload/cross-reference validation. Platform-internal facts are non-gating diagnostic scope.
...
Release 1 lane behavior is limited to delegated authoring records, fake-runtime lane summaries, command/status summaries, and degraded fallback records. Any actual OpenCode subtask/model/provider lane launch requires the managed-dispatch gate: the plugin-verifiable evidence bundle, Guard/user approval for privileged work, configured verification, durable pre-dispatch audit, dispatch idempotency/reservation, adapter-capable intended SDK dispatch path, redaction-safe observed lifecycle/result/status evidence with terminal semantics, and durable evidence reload/cross-reference validation. Trusted runtime echo issuer, platform telemetry correlation, lane conformance, and usage authority attestation are non-gating diagnostic scope only.

Planned top-tier reviewer and perspective bindings for Release 2.5 or Release 3 entry:

1. FlowDesk must support critical review by launching every registered highest-tier available reviewer/model lane when the release gate permits actual lane launch. The highest-tier set is discovered from the active FlowDesk model binding registry and Policy Pack; it is not hard-coded to a fixed provider count. Claude Opus, GPT frontier, and Gemini Pro can be used only as discovery seeds or user-facing family labels. The actual bindings must come from the current daily exact-model availability cache as concrete provider-qualified model ids. Missing unregistered providers do not block review by themselves.
...
4. Each dedicated reviewer binding must resolve through the model binding registry and same-day availability cache to a concrete provider-qualified model id, for example the exact Anthropic, OpenAI, Google, or other provider id currently exposed by the active environment. The registry must reject abstract labels, aliases, stale retired ids, provider-hidden ids, lower-tier substitutions, and account-unavailable ids.
5. All registered highest-tier available reviewer/model bindings are included for high-risk or critical review when fresh auth, usage, quota/reset, provider health, runtime compatibility, observed runtime/status diagnostics, telemetry, and policy evidence all pass. If a registered highest-tier binding is unavailable, FlowDesk records the exclusion reason and safe next action in the inventory snapshot. It must not silently substitute a lower tier, hidden provider, or unregistered model.
6. A top-tier review run must persist redacted telemetry and lane records for each reviewer: requested binding, approved binding, perspective id, non-gating observed diagnostic refs, provider family, model family, usage evidence ref, Provider Health Snapshot ref, output schema status, and final typed review verdict. It must not persist raw prompts, transcripts, provider payloads, runtime echo bodies, raw tool data, raw file contents, or raw paths.
...
11. The model-availability cache must persist only redacted model metadata: checked date/time, cache key inputs, provider family, exact provider-qualified model id, runtime model id where different, highest-tier eligibility refs, availability refs, and safe next actions. It must not persist raw provider catalogs, credentials, token-bearing auth objects, raw API responses, prompts, transcripts, or provider payloads.
...
3. SDK adapter compatibility and plugin/SDK-observable conformance for the new provider/model, command shape, runtime variant, and lane or dispatch surface.
4. Policy Pack eligibility, user approval where required, budget/reserve checks, and professional-boundary checks.
5. Plugin-verifiable binding evidence before dispatch and redaction-safe observed lifecycle/result/status evidence after dispatch. Platform-internal runtime echo remains non-gating diagnostic scope unless a future OpenCode interface makes it plugin-verifiable and a later ADR explicitly promotes it.
6. Plugin-observed status/progress/correlation refs for the enabled real-dispatch and recovery modes; platform-internal telemetry correlation remains non-gating diagnostic scope unless a future OpenCode interface makes it plugin-verifiable and a later ADR explicitly promotes it.
...
Fallback/reselection must be blocked when any required plugin-verifiable input is stale, unknown, refused, shared-limit-suspected, fallback-derived, model-generated, policy-ineligible, SDK-adapter-incompatible, binding/status-ambiguous, unaudited, or missing Guard approval. A fallback-derived usage or health snapshot cannot justify another fallback.
...
11. Runtime/lifecycle/result evidence is collected from plugin-verifiable OpenCode plugin/SDK surfaces, and any platform-internal runtime-echo claim is non-gating diagnostic scope unless a future OpenCode interface makes it plugin-verifiable and a later ADR explicitly promotes it.
...
36. Future managed fallback/reselection is blocked unless fresh provider-native usage, fresh provider health, SDK adapter compatibility, policy eligibility, plugin-verifiable binding and observed lifecycle/result/status evidence, durable pre-dispatch audit, a new attempt id, and explicit Guard approval all pass; platform-internal proof remains non-gating diagnostic scope only unless a future OpenCode interface makes it plugin-verifiable and a later ADR explicitly promotes it.
...
4. Test plugin-verifiable runtime/lifecycle/result observation sources and classify any platform-internal runtime-echo gap as non-gating diagnostic scope.
...
2. Prove plugin-verifiable runtime/lifecycle/result observation from the intended SDK path; keep platform-internal runtime echo as non-gating diagnostic scope unless a future OpenCode interface makes it plugin-verifiable and a later ADR explicitly promotes it.
3. Prove minimum plugin-observed status/progress/correlation surfaces are available within policy tolerance, while surfacing platform-internal telemetry correlation as non-gating diagnostic scope when not verifiable.
...
8. Runtime echo/status is observed through plugin/SDK surfaces; platform-internal runtime echo trust is non-gating diagnostic scope unless a future OpenCode interface makes it plugin-verifiable and a later ADR explicitly promotes it.
...
2. **Npm scope:** `@flowdesk/*` is the normative implementation target for Release 1. Public package publication is now complete for `@flowdesk/core@0.1.0` and `@flowdesk/opencode-plugin@0.1.0`; future scope, visibility, or ownership changes require a newer ADR.
