...
export function validateTopTierReviewBindingInventoryV1(value: unknown): ValidationResult {
  ...
  return combine([
    ...
    validateOpaqueRefArray(value.included_binding_refs, "included_binding_refs", 32),
    validateOpaqueRefArray(value.excluded_binding_refs, "excluded_binding_refs", 32),
    ...
    validateStringArray(value.labels, "labels", undefined, 64),
    validateStringArray(value.blocked_labels, "blocked_labels", undefined, 64),
    validateOpaqueRefArray(value.evidence_refs, "evidence_refs", 64),
    ...
    Array.isArray(value.included_binding_refs) || Array.isArray(value.excluded_binding_refs) || Array.isArray(value.blocked_binding_refs) ? valid() : invalid("inventory binding predicate refs are required"),
    ...
    value.providerCall === false ? valid() : invalid("providerCall must be false for top tier review binding inventory"),
    value.actualLaneLaunch === false ? valid() : invalid("actualLaneLaunch must be false for top tier review binding inventory"),
    value.runtimeExecution === false ? valid() : invalid("runtimeExecution must be false for top tier review binding inventory"),
    value.fallback_authority_enabled === false ? valid() : invalid("fallback_authority_enabled must be false for top tier review binding inventory"),
    value.guard_replacement_authority_enabled === false ? valid() : invalid("guard_replacement_authority_enabled must be false for top tier review binding inventory"),
    value.external_write_authority_enabled === false ? valid() : invalid("external_write_authority_enabled must be false for top tier review binding inventory"),
...
export function validateTopTierReviewerLaneProbeResultV1(value: unknown): ValidationResult {
  ...
  return combine([
    ...
    validateOpaqueRef(value.lane_plan_ref, "lane_plan_ref"),
    isEnumValue(value.channel, TOP_TIER_REVIEWER_LANE_PROBE_CHANNELS) ? valid() : invalid("channel is invalid"),
    validateConcreteProviderQualifiedModelId(value.provider_qualified_model_id, "provider_qualified_model_id"),
    isEnumValue(value.perspective, TOP_TIER_REVIEWER_PERSPECTIVES) ? valid() : invalid("perspective is invalid"),
